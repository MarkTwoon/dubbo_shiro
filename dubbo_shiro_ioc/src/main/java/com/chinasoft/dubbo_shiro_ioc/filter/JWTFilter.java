package com.chinasoft.dubbo_shiro_ioc.filter;

import com.auth0.jwt.exceptions.TokenExpiredException;
import com.chinasoft.dubbo_shiro_ioc.shiro.JWTToken;
...
//            throw new ShiroException(e.getMessage());
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
        ...
        responseError(response,"token手令验证失败");
        return false;
    }
    ...
    protected boolean preHandle(ServletRequest request, ServletResponse response) throws Exception {
        ...
        if (!isLoginAttempt(request,response)){
            responseError(httpServletResponse,"暂无token手令");
            return false;
        }
    ...
    private void responseError(ServletResponse response,String msg){
        ...
        try {
            String rj = new ObjectMapper().writeValueAsString(new ReturnData(401,msg));
