package com.chinasoft.dubbo_shiro_ioc.exception;
...
//    private final Logger logger = LoggerFactory.getLogger(GlobalExceptionHandler.class);
    ...
    public Object handle401(ShiroException e) {
      /*  return new Result(401, "无权访问(Unauthorized):" + e.getMessage());*/
        return new ReturnData(StatusCode.USER_NOT_POWER,"无权访问(Unauthorized):"+ e.getMessage());
    }
    ...
    public Object handle401(UnauthorizedException e) {
      /*  Result result = new Result();*/
        return new ReturnData(StatusCode.USER_NOT_POWER,"无权访问(Unauthorized):当前Subject没有此请求所需权限(" + e.getMessage() + ")");
    ...
    public Object handle401(UnauthenticatedException e) {
        return new ReturnData(StatusCode.USER_NOT_POWER,"无权访问(Unauthorized):当前Subject是匿名Subject，请先登录(This subject is anonymous.)");
    ...
    public Object validException(BindException e) {
        List<FieldError> fieldErrors = e.getBindingResult().getFieldErrors();
        Map<String, Object> error = this.getValidError(fieldErrors);
        return new ReturnData(StatusCode.RETURN_ERROR, error.get("errorList"),error.get("errorMsg").toString());
    ...
    public Object handle(NoHandlerFoundException e) {
        return new ReturnData(StatusCode.DATA_NULL,e.getMessage());
    ...
    public Object globalException(HttpServletRequest request, Throwable ex) {
        return new ReturnData(StatusCode.RETURN_ERROR2,ex.toString() + ": " + ex.getMessage());
